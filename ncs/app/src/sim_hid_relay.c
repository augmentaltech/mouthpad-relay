			   ...
			   struct bt_gatt_discover_params *params)
{
	...
	/* attr is the characteristic declaration; the value handle is +1, and the
	 * CCC descriptor is +2 (the standard layout for a notify characteristic). */
	...
	if (err && err != -EALREADY) {
		...
		subscribed = true;
		LOG_INF("sim HID subscribed (val %u) — normalizing to HOGP",
